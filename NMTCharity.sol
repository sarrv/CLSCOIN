/* A full fledged charity-oriented token which will be going to be the root of an ecosystem*/
...
interface IERC20 {
    ...
    /* this contract DOES NOT gonna make you rich
        it will grow eventually , if you have a patience 
         YOU ARE MOST WELCOME TO THE FAMILY*/
...
library SafeMath {
    ...
    /*WE NEED A TEAM. WHO WANTS TO CHANGE The PEOPLE LOOK at CRYPTOCURRENCY
    IF YOU ARE READING THIS, YOU ARE MOST WELCOME TO JOIN.*/
...
/* CLSCOIN is on a mission to make this world a beautiful place so that we DO NOT have to GO TO THE MOON*/



    contract CLSCOIN is Context, IERC20, Ownable {
    ...
    /* Variables _tTotal, _name, _symbol, _decimals, and numTokensSellToAddToLiquidity
     ...
     audit of Safemoon. SO YEAH, WE DO LEARN FROM SHITCOINS 
  */
    ...
    constructor () public payable{
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);// v2 testnet 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
...
// This make it secure from bots
    ...
    modifier launchRestrict(address sender, address recipient, uint256 amount) {
        if (_tradingTime > now) {
            require(sender == owner() || recipient == owner(), "CLS: transfers are disabled");
        } else if (_tradingTime <= now && _restrictionLiftTime > now) {
            require(amount <= _maxRestrictionAmount, "CLS: amount greater than max limit in restricted mode");
            if (!_isWhitelisted[sender] && !_isWhitelisted[recipient]) {
                require(_lastTx[sender].add(60) <= now && _lastTx[recipient].add(60) <= now, "CLS: only one tx/min in restricted mode");
                _lastTx[sender] = now;
                _lastTx[recipient] = now;
            } else if (!_isWhitelisted[recipient]){
                require(_lastTx[recipient].add(60) <= now, "CLS: only one tx/min in restricted mode");
                _lastTx[recipient] = now;
            } else if (!_isWhitelisted[sender]) {
                require(_lastTx[sender].add(60) <= now, "CLS: only one tx/min in restricted mode");
                _lastTx[sender] = now;
            }
        }
        _;
...
// Bot End here
    ...
    function _transfer(
        ...
        require(!_paused || from == owner() || to == owner(), "CLS: Transfers are paused"); // pause check
    ...
    function buyCLSWithLeftoverBNB(uint256 amount) external onlyOwner() {
      // buy back SAINT with BNB leftover from SwapAndLiquify to increase SAINT price / YES WE CAN DO IT ;)
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        /* "to" account changed to address(this) to mitigate major centralization
         issue. WE DEFINATELY DONT WANT TO CONTROL, COMMUNITY WILL HANDLE EVERYTHING
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        See "SSL-02 | Redundant code" from the Certik audit of SOME SHIT :)
      */
